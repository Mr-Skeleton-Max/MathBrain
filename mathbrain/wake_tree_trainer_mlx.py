...
class TreeWakeTrainerMLX:
    ...
    def run_round(self, dataset: SparseWakeDatasetMLX, verbose: bool = False) -> Dict[str, float]:
        ...
        if n_unique > 0:
            ...
            if _lam is not None:
                gap_mean = 1.0 - ((1 - _lam) * a_strength + _lam * b_strength)
            else:
                gap_mean = 1.0 - (a_strength + b_strength)
            mx.eval(gap_mean)
        else:
    ...
    def run_round_word_gap(self, dataset: SparseWakeDatasetMLX,
        ...
        """Word-level A gap + gap-weighted normalization.

        对每个 raw pair (src, tgt, pos, φ):
          gap_i = 1 - A_word(gold[pos_i]) - B[src,tgt]·φ_i

        对每条 unique edge 聚合:
          delta_edge = η * Σ(gap_i * φ_i) / Σ|gap_i|
        ...
        if n_pairs > 0 and n_unique > 0:
            ...
            # per-pair A word score
            a_word_per_pair = a_word_gold[dataset.pos_ids_sorted_mx]  # (n_pairs,)
            # per-pair gap
            gap = 1.0 - a_word_per_pair - b_strength               # (n_pairs,)

            # gap-weighted phi: gap_i * φ_i
            weighted_delta = gap[:, None] * dataset.phi_mx          # (n_pairs, D)
            abs_gap = mx.abs(gap)                                   # (n_pairs,)

            # scatter-add 聚合到 unique edge (避免 prefix sum 的 float32 精度丢失)
            ...
            sum_gap_phi = sum_gap_phi.at[edge_idx].add(weighted_delta)  # (n_unique, D)
            sum_abs_gap = mx.zeros((n_unique,), dtype=mx.float32)
            sum_abs_gap = sum_abs_gap.at[edge_idx].add(abs_gap)         # (n_unique,)

            # normalized delta per unique edge
            eps = 1e-8
    ...
    def run_round_position_gap(self, dataset: SparseWakeDatasetMLX,
        ...
        gap = mx.where(valid_mask, 1.0 - gold_scores, 0.0)  # (p,)
        mx.eval(gap)
