...
class DenseEvaluatorMLX:
    ...
    def evaluate_dataset(self, dataset) -> EvalResult:
        ...
        # word-level 归一化：每个 position 的 top-1 拉到 1.0
        _normalize = getattr(self, 'normalize_word_scores', False)
        _b_top1_mask = getattr(self, 'b_top1_mask', False)
        ...
        if _normalize:
            ...
            b_max = mx.max(word_scores_b, axis=1, keepdims=True)
            b_max = mx.maximum(b_max, 1e-8)
            word_scores_b = word_scores_b / b_max

        if _b_top1_mask:
            # B 只保留 top-1 word 的分数，其余清零
            b_top1_idx = mx.argmax(word_scores_b, axis=1)  # (p,)
            b_masked = mx.zeros_like(word_scores_b)
            for pos_i in range(p):
                idx = int(b_top1_idx[pos_i])
                b_masked = b_masked.at[pos_i, idx].add(word_scores_b[pos_i, idx])
            word_scores_b = b_masked
